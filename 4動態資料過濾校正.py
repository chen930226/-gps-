    ...
    def _fuse_sources(self, A=None, B=None, C=None):
        ...
        for src in (A, B, C):
            ...
            if la is None or lo is None:
                continue
            if Rm is not None and Rm > self.r_gate:
                continue
            r_eff = max(Rm, 1e-3)
            # 新權重公式
            w = (sat + 0.5 * nsat) / (r_eff**2)
            cand.append((la, lo, w))
        if not cand:
            return None, None
...
for i,row in df.iterrows():
    A = {
    'lat': row["A_Lat"], 'lon': row["A_Lon"], 'R': row["A_R"],
    'Sat': row["A_Sat"], 'NSatView': row["A_NSatView"]
    } if not pd.isna(row["A_Lat"]) else None
    B = {
        'lat': row["B_Lat"], 'lon': row["B_Lon"], 'R': row["B_R"],
        'Sat': row["B_Sat"], 'NSatView': row["B_NSatView"]
    } if not pd.isna(row["B_Lat"]) else None
    C = {
        'lat': row["C_Lat"], 'lon': row["C_Lon"], 'R': row["C_R"],
        'Sat': row["C_Sat"], 'NSatView': row["C_NSatView"]
    } if not pd.isna(row["B_Lat"]) else None
