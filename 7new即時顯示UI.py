...
class SerialReader(threading.Thread):
    ...
    def run(self):
        try:
            ...
            self.running = True
            print(f"[DEBUG] 啟動序列埠讀取：{self.port}")
            while self.running:
                ...
                parts = line.split(',')
                if not parts or "Latitude" in line or "Date" in line:
                    continue
                try:
                    if len(parts) >= 10:
                        lat = float(parts[2])
...
class GPSViewer(QWidget):
    def __init__(self):
        ...
        self.shared_data = {}
        self.timestamps = {}
        self.timestamp_dict = self.timestamps 
    ...
    def init_ui(self):
        ...
        for lbl, color in [(self.label_gps,"red"),(self.label_geometry,"purple"),
                           (self.label_mlp,"green"),(self.label_kf,"orange"),
                           (self.label_sats,"brown")]:
        ...
        ab_box = QGroupBox(f"Raw vs 處理後（{PROCESSED_KIND}）相對準確性")
        ab_grid = QGridLayout()
    ...
    def update_gps_data(self):
        ...
        base_R = 5.0
        avg_r_eff_list = []
        for r, p in zip(rs, COM_PORTS):
            # 使用最近歷史下限抑制 r 爆高
            if p in self.r_history and len(self.r_history[p]) > 0:
                min_hist_r = min(self.r_history[p])
                avg_r_eff_list.append(max(float(r), min_hist_r))
            else:
                avg_r_eff_list.append(float(r))
        avg_r_eff = np.mean(avg_r_eff_list)
        avg_sat = np.mean(sats)

        adaptive_R_value = base_R * (avg_r_eff / 3.0) / max(avg_sat, 1)
        adaptive_R_value = float(np.clip(adaptive_R_value, 2.0, 20.0))
        self.kf.R = np.eye(2) * adaptive_R_value
        ...
        if "IMU" in self.shared_data:
            last_acc = []
            for i in range(3):  # 取 Ax, Ay, Az 最新值
                hist = self.shared_data["IMU"].get(i, [])
                if len(hist) > 0:
                    last_acc.append(abs(hist[-1]))
            acc_mean = float(np.mean(last_acc)) if last_acc else 0.0

        if acc_mean > 1.5:      # 明顯加速/轉彎
            adaptive_Q_value = base_Q * 10.0
        elif acc_mean < 0.3:    # 幾乎靜止
            adaptive_Q_value = base_Q * 0.5
        else:
            adaptive_Q_value = base_Q * 1.0

        self.kf.Q = np.eye(4) * adaptive_Q_value
        ...
        if mlp_xy is not None:
            fused_xy = mlp_xy      # 🚀 直接信任模型結果
        else:
            fused_xy = geo_xy      # ⚙️ 模型失敗時使用幾何法備援

        # --- (3) Innovation gating（異常量測過濾） ---
        if self.kf.x is not None:
            
            z = np.array(fused_xy).reshape(2, 1)
            y = z - (self.kf.H @ self.kf.x)[:2]
            S = self.kf.H @ self.kf.P @ self.kf.H.T + self.kf.R
            mah_dist_matrix = y.T @ np.linalg.inv(S) @ y
            mah_dist = float(np.trace(mah_dist_matrix))  # ✅ 自動取對角線和，只留純量
            if mah_dist > 9.0:  # 約等於 3σ 門檻
                self.kf.R *= 4.0  # 暫時放寬量測不信任
