...
R_GATING_M = 50.0      
UERE_M_DEFAULT = 5.0   # 用於 r = HDOP * UERE 的比例因子
...
class GPSCorrector:
    ...
    def _fuse_sources(self, A: Optional[Dict], B: Optional[Dict], C: Optional[Dict]):
        ...
        for src in (A,B,C):
            ...
            Rm = src.get("R", None)
            sat = src.get("Sat", 1)
            nsat = src.get("NSatView", 0)
            if la is None or lo is None: continue
            if (Rm is not None) and (Rm > self.r_gate): continue
            r_eff = max(Rm, 1e-3)
            w = (sat + 0.5 * nsat) / (r_eff ** 2)   # ← 與 2json 校正一致
            cand.append((la, lo, w))
        if not cand:
            return None, None
        wsum = sum(w for _,_,w in cand)
        lat_w = sum(la*w for la,_,w in cand) / wsum
        lon_w = sum(lo*w for _,lo,w in cand) / wsum
        return lat_w, lon_w
    ...
    def update(self, ts: Optional[float], A: Optional[Dict]=None, B: Optional[Dict]=None, C: Optional[Dict]=None):
        ...
        if do_update:
            ...
            r_values = []
            for src in (A, B, C):
                if src and src.get("R") is not None:
                    r_values.append(src["R"])
            if r_values:
                # 根據最近衛星品質調整觀測雜訊
                r_mean = np.mean(r_values)
                scale = max(0.5, min(2.0, r_mean / UERE_M_DEFAULT))
                R_adapt = self.R * scale
            else:
                R_adapt = self.R

            S = self.H @ self.P @ self.H.T + R_adapt
        ...
        while not stop_flag:
            if ser.in_waiting:
                ...
                if 'ID' in raw:
                    continue
                try:
                    if raw.startswith("Date") or "Latitude" in raw:
                         continue

                    parts = [p.strip() for p in raw.split(',')]
                    # 新格式：Date,Time,Lat,Lon,Alt,Spd,Crs,SatUsed,HDOP,NSatView,SNR
                    if len(parts) >= 10:
                        lat = float(parts[2])
                        lon = float(parts[3])
                        alt = float(parts[4])
                        spd = float(parts[5])
                        crs = float(parts[6])
                        sat_used = int(float(parts[7])) if parts[7] else 0
                        hdop = float(parts[8]) if parts[8] else 0.0
                        nsat = int(float(parts[9])) if parts[9] else 0

                        r = hdop * UERE_M_DEFAULT if hdop > 0 else 10.0
                        sat = sat_used if sat_used > 0 else 1

                        with data_lock:
                            COM_GPS_data[port] = [lat, lon, alt, spd, crs, r, sat, hdop, nsat]
...
EXPECTED_HEADER = [
    'A_Lat','A_Lon','A_Alt','A_Spd','A_Crs','A_R','A_Sat','A_HDOP','A_NSatView',
    'B_Lat','B_Lon','B_Alt','B_Spd','B_Crs','B_R','B_Sat','B_HDOP','B_NSatView',
    'C_Lat','C_Lon','C_Alt','C_Spd','C_Crs','C_R','C_Sat','C_HDOP','C_NSatView',
    'Fused_Lat','Fused_Lon','Corrected_Lat','Corrected_Lon',
    'Roll','Pitch','Yaw','Ax','Ay','Az','Gx','Gy','Gz','Timestamp'
    ...
    while count < 18000:
        ...
        with data_lock:
            ...
            if ready_gps and imu_data_queue:
                ...
                def gps_to_dict(data):
                    return {
                        'lat': data[0], 'lon': data[1],
                        'alt': data[2], 'spd': data[3], 'crs': data[4],
                        'R': data[5], 'Sat': data[6],
                        'HDOP': data[7], 'NSatView': data[8]
                    }

                A = gps_to_dict(ordered[0]) if len(ordered) > 0 else None
                B = gps_to_dict(ordered[1]) if len(ordered) > 1 else None
                C = gps_to_dict(ordered[2]) if len(ordered) > 2 else None
